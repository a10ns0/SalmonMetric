...
├── data/                   # Dataset de imágenes y etiquetas (generado por el script de setup)
...
├── src/                    # Código fuente principal de la aplicación (el "corazón" del proyecto)
│   ├── callbacks/
│   ├── metrics/
│   ├── models/
│   └── utils/
...
Este script prepara el dataset para que sea compatible con YOLOv8. Lee las imágenes y etiquetas de una fuente, las divide en conjuntos de entrenamiento/validación y crea el archivo `data.yaml` necesario.
...
Este script lanza el proceso de *fine-tuning*. Carga la configuración desde `configs/training_config.yaml`, instancia el modelo y los callbacks personalizados, y comienza el entrenamiento.

```
python scripts/01_train.py
```

Los resultados, incluyendo los pesos del modelo (`.pt`) y los logs de métricas, se guardarán en el directorio `outputs/`.

#### Paso 3: Evaluación del Modelo

Utiliza el mejor modelo guardado durante el entrenamiento para realizar una evaluación final sobre el conjunto de prueba.
